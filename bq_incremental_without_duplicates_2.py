...
def get_last_processed_filename(bucket_name, prefix):
    ...
    filename =  blob.name  
    print(f"Filename returned by get_last_processed_filename: {filename}") 
    return filename 
    ...
    if blob.exists():
        ...
        for blob in blobs:
            if blob.name != f'{prefix}/last_processed.txt' and \
               blob.updated > datetime.strptime(last_processed_timestamp, '%Y-%m-%d %H:%M:%S.%f'):
                return blob.name
        return None  
    else:
        return None
...
# DAG definition
with DAG(
    dag_id='GCS_to_BQ_Load_dynamic_2',
    catchup=False,
    schedule_interval=timedelta(days=1),
    default_args=default_args,
    params={
        'project_id': 'profound-surge-418508', 
        'gcs_to_bq_table': 'gcs_to_bq_test_table', 
    }
) as dag:
